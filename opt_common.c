...
extern void fftools_log_callback_function(void *ptr, int level, const char* format, va_list vargs);
...
void show_banner(int argc, char **argv, const OptionDef *options)
{
    ...
    print_all_libs_info(INDENT, AV_LOG_INFO);
...
static void print_codec(const AVCodec *c)
{
    ...
    if (c->ch_layouts) {
        ...
        av_log(NULL, AV_LOG_STDERR, "    Supported channel layouts:");
        while (p->nb_channels) {
            char name[128];
...
static int show_formats_devices(void *optctx, const char *opt, const char *arg, int device_only, int muxdemuxers)
{
    ...
    for (;;) {
        ...
        av_log(NULL, AV_LOG_STDERR, " %c%c %-15s %s\n",
               decode ? 'D' : ' ',
               encode ? 'E' : ' ',
               name,
            long_name ? long_name:" ");
...
__thread int log_callback_report_print_prefix = 1;
...
void log_callback_report(void *ptr, int level, const char *fmt, va_list vl)
{
    ...
    if (report_callback == NULL) {
        ...
        fftools_log_callback_function(ptr, level, fmt, vl);
    }
    av_log_format_line(ptr, level, fmt, vl2, line, sizeof(line), &log_callback_report_print_prefix);
    va_end(vl2);
